...
class Database:
    def __init__(self, uri, database_name):
        ...
        self.weooanimes = self._client[database_name]
        self.col = self.weooanimes.user
    ...
    def new_user(self, id):
        return dict(
            ...
            caption=None,
            metadata=True,
            metadata_code="Telegram : @Weoo_Anime",
    ...
    async def get_title(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('title', 'Encoded by @weoo_anime')
    ...
    async def get_author(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('author', '@weoo_anime')
    ...
    async def get_artist(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('artist', '@weoo_anime')
    ...
    async def get_audio(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('audio', 'By @weoo_anime')
    ...
    async def get_subtitle(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('subtitle', "By @weoo_anime")
    ...
    async def get_video(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('video', 'Encoded By @weoo_anime')

    async def set_video(self, user_id, video):
        await self.col.update_one({'_id': int(user_id)}, {'$set': {'video': video}})


weooanimes = Database(Config.DB_URL, Config.DB_NAME)
